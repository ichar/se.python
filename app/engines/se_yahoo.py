...
__all__ = ['YahooSearchEngine', 'search']
...
_section = Config['YAHOO']

_urls = { \
    'home'          : "http://search.yahoo.%(tld)s/",
    'search'        : "http://search.yahoo.%(tld)s/search?p=%(query)s&pz=%(num)d&fr=sfp&fr2=sb-top-search&b=%(start)d&bct=0&xargs=0",
    'next_page'     : "http://search.yahoo.%(tld)s/search?p=%(query)s&pz=%(num)d&fr=sfp&fr2=sb-top-search&b=%(start)d&bct=0&xargs=0",
    'search_num'    : "http://search.yahoo.%(tld)s/search?p=%(query)s&pz=%(num)d&fr=sfp&fr2=sb-top-search&b=%(start)d&bct=0&xargs=0",
    'next_page_num' : "http://search.yahoo.%(tld)s/search?p=%(query)s&pz=%(num)d&fr=sfp&fr2=sb-top-search&b=%(start)d&bct=0&xargs=0",
}
...
class YahooSearchEngine(BaseSearchEngine):
    ...
    def __init__(self, alias, urls, timeout, **kw):
        super(YahooSearchEngine, self).__init__(alias, urls, timeout, **kw)


def search(query, logger, errorlog, **kw):
    silence = kw.get('silence') and True or False

    se = YahooSearchEngine(_section.get('alias'), _urls, _defailt_timeout, 
...
if __name__ == "__main__":
    ...
    formatter = BannerHelpFormatter(
        "Python script to use the Yahoo search engine\n"
