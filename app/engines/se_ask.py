...
__all__ = ['AskSearchEngine', 'search']
...
_urls = { \
    'home'          : "http://www.ask.%(tld)s/",
    'search'        : "http://www.ask.%(tld)s/web?q=%(query)s&qsrc=0&o=0&l=dir&qo=homepageSearchBox",
    'next_page'     : "http://www.ask.%(tld)s/web?q=%(query)s&page=%(page)d&qsrc=0&o=0&l=dir&qo=pagination",
    'search_num'    : "http://www.ask.%(tld)s/web?q=%(query)s&page=%(page)d&qsrc=0&o=0&l=dir&qo=homepageSearchBox",
    'next_page_num' : "http://www.ask.%(tld)s/web?q=%(query)s&page=%(page)d&qsrc=0&o=0&l=dir&qo=pagination",
}

_defailt_timeout = _section.getfloat('timeout')


class AskSearchEngine(BaseSearchEngine):

    def __init__(self, alias, urls, timeout, **kw):
        super(AskSearchEngine, self).__init__(alias, urls, timeout, **kw)


def search(query, logger, errorlog, **kw):
    silence = kw.get('silence') and True or False

    se = AskSearchEngine(_section.get('alias'), _urls, _defailt_timeout, 
...
if __name__ == "__main__":
    ...
    formatter = BannerHelpFormatter(
        "Python script to use the Ask search engine\n"
