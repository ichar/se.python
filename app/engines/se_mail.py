...
__all__ = ['MailSearchEngine', 'search']
...
_section = Config['MAIL']

_urls = { \
    'home'          : "http://go.mail.%(tld)s/",
    'search'        : "http://go.mail.%(tld)s/search?tsg=l&fr=main&q=%(query)s",
    'next_page'     : "http://go.mail.%(tld)s/search?q=%(query)s&sf=%(start)d",
    'search_num'    : "http://go.mail.%(tld)s/search?q=%(query)s&sf=%(start)d",
    'next_page_num' : "http://go.mail.%(tld)s/search?q=%(query)s&sf=%(start)d",
}

_defailt_timeout = _section.getfloat('timeout')


class MailSearchEngine(BaseSearchEngine):

    def __init__(self, alias, urls, timeout, **kw):
        super(MailSearchEngine, self).__init__(alias, urls, timeout, **kw)


def search(query, logger, errorlog, **kw):
    silence = kw.get('silence') and True or False

    se = MailSearchEngine(_section.get('alias'), _urls, _defailt_timeout, 
