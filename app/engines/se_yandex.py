...
__all__ = ['YandexSearchEngine', 'search']
...
_section = Config['YANDEX']

_urls = { \
    'home'          : "http://www.yandex.%(tld)s/",
    'search'        : "http://www.yandex.%(tld)s/search?text=%(query)s",
    'next_page'     : "http://yandex.%(tld)s/search?text=%(query)s&p=%(page)s",
    'search_num'    : "http://www.yandex.%(tld)s/search?text=%(query)s",
    'next_page_num' : "http://yandex.%(tld)s/search?text=%(query)s&p=%(page)s",
}
...
class YandexSearchEngine(BaseSearchEngine):
    ...
    def __init__(self, alias, urls, timeout, **kw):
        super(YandexSearchEngine, self).__init__(alias, urls, timeout, **kw)


def search(query, logger, errorlog, **kw):
    silence = kw.get('silence') and True or False

    se = YandexSearchEngine(_section.get('alias'), _urls, _defailt_timeout, 
