...
__all__ = ['RamblerSearchEngine', 'search']
...
_section = Config['RAMBLER']

_urls = { \
    'home'          : "http://nova.rambler.%(tld)s/",
    'search'        : "http://nova.rambler.%(tld)s/search?scroll=1&utm_source=nhp&utm_content=search&utm_medium=enter&utm_campaign=self_promo&query=%(query)s",
    'next_page'     : "http://nova.rambler.%(tld)s/search?scroll=1&utm_source=nhp&utm_content=search&utm_medium=enter&utm_campaign=self_promo&query=%(query)s&page=%(page)s",
    'search_num'    : "http://nova.rambler.%(tld)s/search?scroll=1&utm_source=nhp&utm_content=search&utm_medium=enter&utm_campaign=self_promo&query=%(query)s&page=%(page)s",
    'next_page_num' : "http://nova.rambler.%(tld)s/search?scroll=1&utm_source=nhp&utm_content=search&utm_medium=enter&utm_campaign=self_promo&query=%(query)s&page=%(page)s",
}

_defailt_timeout = _section.getfloat('timeout')


class RamblerSearchEngine(BaseSearchEngine):

    def __init__(self, alias, urls, timeout, **kw):
        super(RamblerSearchEngine, self).__init__(alias, urls, timeout, **kw)


def search(query, logger, errorlog, **kw):
    silence = kw.get('silence') and True or False

    se = RamblerSearchEngine(_section.get('alias'), _urls, _defailt_timeout, 
