...
__all__ = ['GoogleSearchEngine', 'search']
...
_section = Config['GOOGLE']

_urls = { \
    'home'          : "http://www.google.%(tld)s/",
    'search'        : "http://www.google.%(tld)s/search?hl=%(lang)s&q=%(query)s&btnG=Google+Search&inurl=https",
    'next_page'     : "http://www.google.%(tld)s/search?hl=%(lang)s&q=%(query)s&start=%(start)d&inurl=https",
    'search_num'    : "http://www.google.%(tld)s/search?hl=%(lang)s&q=%(query)s&num=%(num)d&btnG=Google+Search&inurl=https",
    'next_page_num' : "http://www.google.%(tld)s/search?hl=%(lang)s&q=%(query)s&num=%(num)d&start=%(start)d&inurl=https",
}
...
class GoogleSearchEngine(BaseSearchEngine):
    ...
    def __init__(self, alias, urls, timeout, **kw):
        super(GoogleSearchEngine, self).__init__(alias, urls, timeout, **kw)


def search(query, logger, errorlog, **kw):
    silence = kw.get('silence') and True or False

    se = GoogleSearchEngine(_section.get('alias'), _urls, _defailt_timeout, 
